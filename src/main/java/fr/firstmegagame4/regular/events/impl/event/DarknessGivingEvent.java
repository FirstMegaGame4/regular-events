...
public class DarknessGivingEvent implements RegularEvent {
	...
	public Text getEventDisplayName() {
		return Text.of("Darkness Giving Event");
	}

	@Override
	public Text getEventDescription() {
		return Text.of("Give the darkness effect randomly on one player");
	}
	...
	public void execute(MinecraftServer server, List<ServerPlayerEntity> players) {
		EventUtil.playerRandomAction(players, server, (world, player) -> {
			...
			EventUtil.shout(server, Objects.requireNonNull(player.getDisplayName()).copy().append(" just received darkness!"));
		});
	}
}
