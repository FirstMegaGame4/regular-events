...
public class CobwebBlowingEvent implements RegularEvent {
	...
	public Text getEventDisplayName() {
		return Text.of("Cobweb Blowing Event");
	}

	@Override
	public Text getEventDescription() {
		return Text.of("Get stuck niark");
	}
	...
	public void execute(MinecraftServer server, List<ServerPlayerEntity> players) {
		EventUtil.playerRandomAction(players, server, (world, player) -> {
			...
			EventUtil.shout(server, Objects.requireNonNull(player.getDisplayName()).copy().append(" has been cobweb blew!"));
		});
	}
}
